...
struct genericMoeGemmKernelLauncher {
  static void call(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs,
                   int sm_count_) {
...
#else
    static_assert(
        cutlass::platform::is_same<T, half>::value || cutlass::platform::is_same<T, float>::value,
    ...
    static_assert(arch::kMinComputeCapability < 90,
                  "Sm90+ architecture should use specialized kernels");
    ...
    if (!inputs.use_fused_moe) {
      ...
      int const gemm_group_size = QuantOp == cutlass::WeightOnlyQuantOp::UNDEFINED
                                      ? inputs.k
                                      : inputs.groupwise_quant_group_size;
...
static void dispatch(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs,
  ...
  if constexpr ((Stages == 2 || Arch::kMinComputeCapability >= 80) &&
    ...
    TLLM_THROW("Cutlass gemm. Not instantiated for arch %d with stages set to %d",
...
template <typename T, typename WeightType, typename GemmOutputType, typename arch,
          ...
          typename std::enable_if<(std::is_same<T, __nv_fp8_e4m3>::value ||
...
void dispatchGemmConfig(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs,
  ...
  switch (inputs.gemm_config.stages) {
    ...
    case 3:
      ...
      break;
    case 4:
      dispatch<T, WeightType, GemmOutputType, arch, EpilogueTag, ThreadblockShape, WarpShape, 4>(
          inputs, sm_count_);
      break;
    default:
      TLLM_THROW("dispatchGemmConfig does not support stages %d", inputs.gemm_config.stages);
      break;
  }
}
...
#endif
                                  && std::is_same<T, WeightType>::value>::type* = nullptr>
void dispatchMoeGemmToCutlass(
    GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_) {
...
// Overload for quantize MoE GEMMs. We disable some warp configs here since they will not be used
// and we can improve compile time
template <typename T, typename WeightType, typename GemmOutputType, typename arch,
          typename EpilogueTag,
          typename std::enable_if<!std::is_same<T, float>::value &&
                                  !std::is_same<T, WeightType>::value>::type* = nullptr>
  ...
  switch (inputs.gemm_config.tile_config_sm80) {
    case cutlass_extensions::CutlassTileConfig::CtaShape16x128x64_WarpShape16x32x64:
      ...
      if constexpr (arch::kMinComputeCapability >= 75) {
        dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag,
                           ...
                           cutlass::gemm::GemmShape<16, 32, tile_shape_k>>(inputs, sm_count_);
      }
      break;
    case cutlass_extensions::CutlassTileConfig::CtaShape16x256x64_WarpShape16x64x64:
      TLLM_CHECK_WITH_INFO(arch::kMinComputeCapability >= 75, "Invalid config on Volta");
      if constexpr (arch::kMinComputeCapability >= 75) {
        dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag,
                           ...
                           cutlass::gemm::GemmShape<16, 64, tile_shape_k>>(inputs, sm_count_);
      }
      break;
    case cutlass_extensions::CutlassTileConfig::CtaShape32x128x64_WarpShape32x32x64:
      dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag,
                         cutlass::gemm::GemmShape<32, 128, tile_shape_k>,
                         cutlass::gemm::GemmShape<32, 32, tile_shape_k>>(inputs, sm_count_);
      break;
    case cutlass_extensions::CutlassTileConfig::CtaShape64x128x64_WarpShape64x32x64:
      dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag,
                         cutlass::gemm::GemmShape<64, 128, tile_shape_k>,
                         cutlass::gemm::GemmShape<64, 32, tile_shape_k>>(inputs, sm_count_);
      break;
    case cutlass_extensions::CutlassTileConfig::CtaShape128x128x64_WarpShape128x32x64:
      dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag,
                         ...
                         cutlass::gemm::GemmShape<128, 32, tile_shape_k>>(inputs, sm_count_);
      break;
    case cutlass_extensions::CutlassTileConfig::Undefined:
      TLLM_THROW("GEMM config undefined.");
      break;
