...
inline cudaError_t TwoStageHolisticPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
  ...
  for (uint32_t task = 0; task < NUM_TASKS; ++task) {
    ...
    if (cluster_tile_q >= 64) {
      ...
      kv_len_limit = f(std::max(ceil_div(total_kv_lens, num_clusters), (int64_t) 1L));
    } else {
      kv_len_limit = f(std::max(ceil_div(total_kv_lens * num_kv_heads, num_clusters), (int64_t) 1L));
    }
